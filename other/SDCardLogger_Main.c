...
/* USER CODE BEGIN PV */
volatile uint16_t adc_value; /* ADC value */

/**** Timer related ****/
uint32_t dht_millis=0, timer_millis=0;
char timer_buffer[9];

/**** DHT22 related ****/
#define DHT22_FREQ 20000 /* Get data every 1 minute */
float temperature=20, humidity=50;
char temp_buffer_sd[200], temp_buffer_lcd[200];

/**** SD Card related ****/
FIL accelerometer, thermohygrometer; // File
...
int main(void)
{
  ...
  if(f_mount(&SDFatFS, (TCHAR const*)SDPath, 1) != FR_OK) /* Mount uSD card */
  	...
  	if(f_open(&thermohygrometer, "TEMP.TXT", FA_OPEN_APPEND | FA_WRITE) != FR_OK) /* Open TXT file */
  		printf("Can't create file\r\n");
  	else {
  		fresult = f_write(&thermohygrometer, start_buffer, strlen((char *)start_buffer), (void *)&byteswritten); /* Write to TXT file */
  		printf("TEMP.txt created and the data is written \r\n");
...
//  Lis3dhInit(&dev_ctx, hi2c3);
  ...
  while (1)
  {
  	...
  	acceleration_mg[2] = (lis3dh_from_fs4_hr_to_mg(data_raw_acceleration[2]));//-(-48))*1.4359 + (-441.6);
  	sprintf(acc_buffer, "%li, %.2f\t\r\n", HAL_GetTick(), acceleration_mg[2]);
  	printf(acc_buffer);
#endif

#if defined(DHT)
  	/* Get temperature/humidity data and log to TXT file */
  	if (HAL_GetTick() - dht_millis >= DHT22_FREQ){
  		dht_millis = HAL_GetTick();

//  		Dht22(GPIOC, GPIO_PIN_0, &temperature, &humidity); /* Read temperature and humidity from DHT22 */
  		temperature = TMP117_get_Temperature(hi2c3);

  		/* Create string to write to file */
  		sprintf(temp_buffer_sd, "%li, %02lu:%02lu, %.1f, %.1f\r\n", dht_millis, (dht_millis/3600000)%24, (dht_millis/60000)%60, temperature, humidity); /* Data format "HH:MM, Temperature, Humidity" */
  		sprintf(temp_buffer_lcd, "%i %.1f %.1f", div(dht_millis,1000).quot, temperature, humidity); /* Data format "Second, Temperature, Humidity" */

  		/* Logging to TXT file and debugging */
  		fresult = f_open(&thermohygrometer, "TEMP.TXT", FA_OPEN_APPEND | FA_WRITE); /* Open TXT file */
  		if (fresult != FR_OK){
//  			sprintf(error_buffer, "ERROR! File could not be opened! Fresuslt is %i \r\n",fresult);
//  			printf(error_buffer);
  			LcdSendStringAtPos(0, 15, "E");
  		}
  		else {
//  			printf("File is opened successfully!\r\n");
  			LcdSendStringAtPos(0,15,"O");
